/*  =========================================================================
    ...
    * The XML model used for this code generation: nmap_msg.xml
...
struct _nmap_msg_t {
    ...
    zmsg_t *ports;                      //  List of port_scan results. Each frame is nmap_msg_port_scan_t
    zmsg_t *os;                         //  List of os_scan results. Each frame is nmap_msg_os_scan_t
    zmsg_t *scripts;                    //  List of script results. Each frame is nmap_msg_script_t
    ...
    zmsg_t *service;                    //  Service detected on a port (optional, nmap_msg_service_scan_t)
    ...
    zmsg_t *portused;                   //  List of portused results. Each frame is nmap_msg_portused_t
    zmsg_t *osmatch;                    //  List of osmatch results. Each frame is nmap_msg_osmatch_t
    zmsg_t *osclass;                    //  List of osclass results. Each frame is nmap_msg_osscan_t
...
nmap_msg_destroy (nmap_msg_t **self_p)
{
    ...
    if (*self_p) {
        ...
        zmsg_destroy (&self->ports);
        zmsg_destroy (&self->os);
        zmsg_destroy (&self->scripts);
        free (self->protocol);
        free (self->reason_ip);
        zmsg_destroy (&self->service);
        free (self->name);
        free (self->version);
        free (self->product);
        free (self->extrainfo);
        ...
        zmsg_destroy (&self->portused);
        zmsg_destroy (&self->osmatch);
        zmsg_destroy (&self->osclass);
        free (self->proto);
...
nmap_msg_decode (zmsg_t **msg_p)
{
    ...
    switch (self->id) {
        ...
        case NMAP_MSG_DEV_SCAN:
            ...
            self->ports = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->ports, zmsg_pop (msg));
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->os = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->os, zmsg_pop (msg));
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->scripts = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->scripts, zmsg_pop (msg));
            break;
        ...
        case NMAP_MSG_PORT_SCAN:
            ...
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->service = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->service, zmsg_pop (msg));
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->scripts = zmsg_new ();
            while (zmsg_size (msg))
        ...
        case NMAP_MSG_OS_SCAN:
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->portused = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->portused, zmsg_pop (msg));
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->osmatch = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->osmatch, zmsg_pop (msg));
            //  Get zero or more remaining frames, leaving current
            //  frame untouched
            self->osclass = zmsg_new ();
            while (zmsg_size (msg))
                zmsg_add (self->osclass, zmsg_pop (msg));
            break;
...
nmap_msg_encode (nmap_msg_t **self_p)
{
    ...
    switch (self->id) {
        ...
        case NMAP_MSG_LIST_SCAN:
            ...
            if (self->hostnames) {
                ...
                while (item) {
                    self->hostnames_bytes += 1 + strlen (zhash_cursor (self->hostnames));
        ...
        case NMAP_MSG_DEV_SCAN:
            ...
            if (self->addresses) {
                ...
                while (item) {
                    self->addresses_bytes += 1 + strlen (zhash_cursor (self->addresses));
    ...
    switch (self->id) {
        ...
        case NMAP_MSG_LIST_SCAN:
            ...
            if (self->hostnames) {
                ...
                char *item = (char *) zhash_first (self->hostnames);
                while (item) {
                    PUT_STRING (zhash_cursor (self->hostnames));
                    PUT_LONGSTR (item);
                    item = (char *) zhash_next (self->hostnames);
                }
            }
        ...
        case NMAP_MSG_DEV_SCAN:
            ...
            if (self->addresses) {
                ...
                char *item = (char *) zhash_first (self->addresses);
                while (item) {
                    PUT_STRING (zhash_cursor (self->addresses));
                    PUT_LONGSTR (item);
                    item = (char *) zhash_next (self->addresses);
                }
            }
    ...
    //  Now send the ports field if set
    if (self->id == NMAP_MSG_DEV_SCAN) {
        zframe_t *ports_part = zmsg_pop (self->ports);
        while (ports_part) {
            zmsg_append (msg, &ports_part);
            ports_part = zmsg_pop (self->ports);
        }
    }
    //  Now send the os field if set
    if (self->id == NMAP_MSG_DEV_SCAN) {
        zframe_t *os_part = zmsg_pop (self->os);
        while (os_part) {
            zmsg_append (msg, &os_part);
            os_part = zmsg_pop (self->os);
        }
    }
    //  Now send the scripts field if set
    if (self->id == NMAP_MSG_DEV_SCAN) {
        zframe_t *scripts_part = zmsg_pop (self->scripts);
        while (scripts_part) {
            zmsg_append (msg, &scripts_part);
            scripts_part = zmsg_pop (self->scripts);
        }
    }
    //  Now send the service field if set
    if (self->id == NMAP_MSG_PORT_SCAN) {
        zframe_t *service_part = zmsg_pop (self->service);
        while (service_part) {
            zmsg_append (msg, &service_part);
            service_part = zmsg_pop (self->service);
        }
    }
    //  Now send the scripts field if set
    if (self->id == NMAP_MSG_PORT_SCAN) {
    ...
    //  Now send the portused field if set
    if (self->id == NMAP_MSG_OS_SCAN) {
        zframe_t *portused_part = zmsg_pop (self->portused);
        while (portused_part) {
            zmsg_append (msg, &portused_part);
            portused_part = zmsg_pop (self->portused);
        }
    }
    //  Now send the osmatch field if set
    if (self->id == NMAP_MSG_OS_SCAN) {
        zframe_t *osmatch_part = zmsg_pop (self->osmatch);
        while (osmatch_part) {
            zmsg_append (msg, &osmatch_part);
            osmatch_part = zmsg_pop (self->osmatch);
        }
    }
    //  Now send the osclass field if set
    if (self->id == NMAP_MSG_OS_SCAN) {
        zframe_t *osclass_part = zmsg_pop (self->osclass);
        while (osclass_part) {
            zmsg_append (msg, &osclass_part);
            osclass_part = zmsg_pop (self->osclass);
        }
    }
    ...
    zmsg_t *scripts)
{
    ...
    zhash_t *hostnames_copy = zhash_dup (hostnames);
    nmap_msg_set_hostnames (self, &hostnames_copy);
    zmsg_t *ports_copy = zmsg_dup (ports);
    nmap_msg_set_ports (self, &ports_copy);
    zmsg_t *os_copy = zmsg_dup (os);
    nmap_msg_set_os (self, &os_copy);
    zmsg_t *scripts_copy = zmsg_dup (scripts);
    ...
    zmsg_t *scripts)
{
    ...
    zmsg_t *service_copy = zmsg_dup (service);
    ...
    zmsg_t *osclass)
{
    ...
    zmsg_t *portused_copy = zmsg_dup (portused);
    nmap_msg_set_portused (self, &portused_copy);
    zmsg_t *osmatch_copy = zmsg_dup (osmatch);
    nmap_msg_set_osmatch (self, &osmatch_copy);
    zmsg_t *osclass_copy = zmsg_dup (osclass);
...
nmap_msg_dup (nmap_msg_t *self)
{
    ...
    switch (self->id) {
        ...
        case NMAP_MSG_DEV_SCAN:
            ...
            copy->hostnames = self->hostnames? zhash_dup (self->hostnames): NULL;
            copy->ports = self->ports? zmsg_dup (self->ports): NULL;
            copy->os = self->os? zmsg_dup (self->os): NULL;
            copy->scripts = self->scripts? zmsg_dup (self->scripts): NULL;
            break;
        ...
        case NMAP_MSG_PORT_SCAN:
            ...
            copy->service = self->service? zmsg_dup (self->service): NULL;
            copy->scripts = self->scripts? zmsg_dup (self->scripts): NULL;
            break;
        ...
        case NMAP_MSG_OS_SCAN:
            copy->portused = self->portused? zmsg_dup (self->portused): NULL;
            copy->osmatch = self->osmatch? zmsg_dup (self->osmatch): NULL;
            copy->osclass = self->osclass? zmsg_dup (self->osclass): NULL;
            break;
...
nmap_msg_print (nmap_msg_t *self)
{
    ...
    switch (self->id) {
        ...
        case NMAP_MSG_OS_SCAN:
            ...
            zsys_debug ("    portused=");
            if (self->portused)
                zmsg_print (self->portused);
            else
                zsys_debug ("(NULL)");
            zsys_debug ("    osmatch=");
            if (self->osmatch)
                zmsg_print (self->osmatch);
            else
                zsys_debug ("(NULL)");
            zsys_debug ("    osclass=");
            if (self->osclass)
                zmsg_print (self->osclass);
            else
                zsys_debug ("(NULL)");
            break;
...
void
nmap_msg_set_ports (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->ports);
...
void
nmap_msg_set_os (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->os);
...
void
nmap_msg_set_scripts (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->scripts);
...
void
nmap_msg_set_service (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->service);
...
void
nmap_msg_set_portused (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->portused);
...
void
nmap_msg_set_osmatch (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->osmatch);
...
void
nmap_msg_set_osclass (nmap_msg_t *self, zmsg_t **msg_p)
{
    assert (self);
    assert (msg_p);
    zmsg_destroy (&self->osclass);
...
nmap_msg_test (bool verbose)
{
    ...
    nmap_msg_hostnames_insert (self, "Name", "Brutus");
    nmap_msg_hostnames_insert (self, "Age", "%d", 43);
    zmsg_t *dev_scan_ports = zmsg_new ();
    nmap_msg_set_ports (self, &dev_scan_ports);
    zmsg_addstr (nmap_msg_ports (self), "Hello, World");
    zmsg_t *dev_scan_os = zmsg_new ();
    nmap_msg_set_os (self, &dev_scan_os);
    zmsg_addstr (nmap_msg_os (self), "Hello, World");
    zmsg_t *dev_scan_scripts = zmsg_new ();
    nmap_msg_set_scripts (self, &dev_scan_scripts);
    zmsg_addstr (nmap_msg_scripts (self), "Hello, World");
    //  Send twice from same object
    nmap_msg_send_again (self, output);
    nmap_msg_send (&self, output);
    ...
    for (instance = 0; instance < 2; instance++) {
        ...
        assert (streq (nmap_msg_hostnames_string (self, "Name", "?"), "Brutus"));
        assert (nmap_msg_hostnames_number (self, "Age", 0) == 43);
        assert (zmsg_size (nmap_msg_ports (self)) == 1);
        assert (zmsg_size (nmap_msg_os (self)) == 1);
        assert (zmsg_size (nmap_msg_scripts (self)) == 1);
        nmap_msg_destroy (&self);
    }
    ...
    zmsg_t *port_scan_service = zmsg_new ();
    nmap_msg_set_service (self, &port_scan_service);
    zmsg_addstr (nmap_msg_service (self), "Hello, World");
    zmsg_t *port_scan_scripts = zmsg_new ();
    ...
    for (instance = 0; instance < 2; instance++) {
        ...
        assert (zmsg_size (nmap_msg_service (self)) == 1);
        assert (zmsg_size (nmap_msg_scripts (self)) == 1);
        nmap_msg_destroy (&self);
    }
    ...
    zmsg_t *os_scan_portused = zmsg_new ();
    nmap_msg_set_portused (self, &os_scan_portused);
    zmsg_addstr (nmap_msg_portused (self), "Hello, World");
    zmsg_t *os_scan_osmatch = zmsg_new ();
    nmap_msg_set_osmatch (self, &os_scan_osmatch);
    zmsg_addstr (nmap_msg_osmatch (self), "Hello, World");
    zmsg_t *os_scan_osclass = zmsg_new ();
    nmap_msg_set_osclass (self, &os_scan_osclass);
    zmsg_addstr (nmap_msg_osclass (self), "Hello, World");
    //  Send twice from same object
    nmap_msg_send_again (self, output);
    nmap_msg_send (&self, output);
    ...
    for (instance = 0; instance < 2; instance++) {
        ...
        assert (zmsg_size (nmap_msg_portused (self)) == 1);
        assert (zmsg_size (nmap_msg_osmatch (self)) == 1);
        assert (zmsg_size (nmap_msg_osclass (self)) == 1);
        nmap_msg_destroy (&self);
    }
