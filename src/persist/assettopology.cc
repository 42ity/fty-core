...
zframe_t* select_childs(const char* url, uint32_t element_id, uint32_t element_type_id, uint32_t child_type_id, bool is_recursive, uint32_t current_depth, uint32_t filtertype)
{
    ...
    assert ( ( filtertype >= 1 ) && ( filtertype <= 7 ) ); // it can be only 1,2,3,4,5,6.7. 7 means - take all
    ...
    try{
        ...
        tntdb::Statement st = conn.prepareCached(
            ...
            " WHERE v.id_parent = :elementid AND v.id_parent_type = :elementtypeid and v.id_type = :childtypeid"
        );
        ...
        for ( auto &row: result )
        {
            ...
            if ( ( is_recursive ) && ( child_type_id != DEVICE ) && ( current_depth <= MAX_RECURSION_DEPTH ) )
            {
                if  ( ( child_type_id < 2 ) && ( 2 <= filtertype ) )
                {
                    log_info ("start select_dcs\n");
                    dcs     = select_childs (url, id, child_type_id, 2, is_recursive, current_depth + 1, filtertype);
                    log_info ("end select_dcs\n");
                }
                if ( ( child_type_id < 3 ) && ( 3 <= filtertype ) )
                {
                    log_info ("start select_rooms\n");
                    rooms   = select_childs (url, id, child_type_id, 3, is_recursive, current_depth + 1, filtertype);
                    log_info ("end select_rooms\n");
                }
                if ( ( child_type_id < 4 ) && ( 4 <= filtertype ) )
                {
                    log_info ("start select_rows\n");
                    rows    = select_childs (url, id, child_type_id, 4, is_recursive, current_depth + 1, filtertype);
                    log_info ("end select_rows\n");
                }
                if ( ( child_type_id < 5 ) && ( 5 <= filtertype ) )
                {
                    log_info ("start select_racks\n");
                    racks   = select_childs (url, id, child_type_id, 5, is_recursive, current_depth + 1, filtertype);
                    log_info ("end select_racks\n");
                }
                if ( ( child_type_id < 6 ) && ( 6 <= filtertype ) )
                {
                    log_info ("start select_devices\n");
                    devices = select_childs (url, id, child_type_id, 6, is_recursive, current_depth + 1, filtertype);
                    log_info ("end select_devices\n");
                }
                if ( child_type_id < 2 )
                {
                    log_info ("start select_grps\n");
                    grps    = select_childs (url, id, child_type_id, 1, is_recursive, current_depth + 1, filtertype);
                    log_info ("end select_grps\n");
                }
            }
            if (!( ( filtertype < 7 ) &&
                 ( ( my_size(dcs) == 0 ) && ( my_size(rooms) == 0 ) && ( my_size(rows) == 0 ) 
                                && ( my_size(racks) == 0 ) && ( my_size(devices) == 0 ) && (my_size(grps) ==0 ) 
                 ) &&
                 ( child_type_id != filtertype ) 
                ))
            {
                zmsg_t* el = asset_msg_encode_return_location_from 
                                (id, id_type, name.c_str(), dcs, rooms, rows, racks, devices, grps);
                assert (el);
...
zmsg_t* get_return_topology(const char* url, asset_msg_t* getmsg)
{
    assert ( getmsg );
    log_info("start\n");
    ...
    {
        log_info ("start select_dcs\n");
        dcs     = select_childs (url, element_id, type_id, 2, is_recursive, 1, filter_type);
        if ( dcs == NULL )
        {
            log_warning("end abnormal\n");
            return common_msg_encode_fail (BIOS_ERROR_DB, DB_ERROR_INTERNAL, "dcs error", NULL);
        }
        log_info ("end select_dcs\n");
    }
    ...
    {
        log_info ("start select_rooms\n");
        rooms   = select_childs (url, element_id, type_id, 3, is_recursive, 1, filter_type);
        if ( rooms == NULL )
        {
            zframe_destroy (&dcs);
            log_warning("end abnormal\n");
            return common_msg_encode_fail (BIOS_ERROR_DB, DB_ERROR_INTERNAL, "rooms error", NULL);
        }
        log_info ("end select_rooms\n");
    }
    ...
    {
        log_info ("start select_rows\n");
        rows    = select_childs (url, element_id, type_id, 4, is_recursive, 1, filter_type);
        if ( rows == NULL )
        {
            zframe_destroy (&dcs);
            zframe_destroy (&rooms);
            log_warning("end abnormal\n");
            return common_msg_encode_fail (BIOS_ERROR_DB, DB_ERROR_INTERNAL, "rows error", NULL);
        }
        log_info ("end select_rows\n");
    }
    ...
    {
        log_info ("start select_racks\n");
        racks   = select_childs (url, element_id, type_id, 5, is_recursive, 1, filter_type);
        if ( racks == NULL )
        {
            zframe_destroy (&dcs);
            zframe_destroy (&rooms);
            zframe_destroy (&rows);
            log_warning("end abnormal\n");
            return common_msg_encode_fail (BIOS_ERROR_DB, DB_ERROR_INTERNAL, "racks error", NULL);
        }
        log_info ("end select_racks\n");
    }
    ...
    {
        log_info ("start select_devices\n");
        devices = select_childs (url, element_id, type_id, 6, is_recursive, 1, filter_type );
        if ( devices == NULL )
        {
            zframe_destroy (&dcs);
            zframe_destroy (&rooms);
            ...
            zframe_destroy (&racks);
            log_warning("end abnormal\n");
            return common_msg_encode_fail (BIOS_ERROR_DB, DB_ERROR_INTERNAL, "devices error", NULL);
        }
        log_info ("end select_devices\n");
    }
    if ( type_id < 2 )
    {
        log_info ("start select_grps\n");
        grps    = select_childs (url, element_id, type_id, 1, is_recursive, 1, filter_type);
        if ( grps == NULL )
        {
            zframe_destroy (&dcs);
            zframe_destroy (&rooms);
            ...
            zframe_destroy (&devices);
            log_warning("end abnormal\n");
            return common_msg_encode_fail (BIOS_ERROR_DB, DB_ERROR_INTERNAL, "groups error", NULL);
        }
        log_info ("end select_grps\n");
    }
    zmsg_t* el = asset_msg_encode_return_location_from 
                        (element_id, type_id, name.c_str(), dcs, rooms, rows, racks, devices, grps);
    log_info("end normal\n");
    return el;
}
