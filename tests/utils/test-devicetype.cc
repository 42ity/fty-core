...
TEST_CASE("device type getters1","[dbdevtype][constructor1][toString][getId][getUrl][getState][getName]"){
    utils::db::DeviceType dbdevicetype(url);
    ...
    REQUIRE( utils::db::objectStatetoString(dbdevicetype.getState()) == osnew );
...
TEST_CASE("device type getters2","[dbdevtype][constructor2][toString][getId][getUrl][getState][getName]"){
    ...
    utils::db::DeviceType dbdevicetype(url,name);
    std::string expected =  "url="   + url                + ";" +
                            "id="    + std::to_string(-1)   + ";" +
                            "state=" + osnew              + ";" +
                            "name="  + name                 ;
...
TEST_CASE("device type selectbyname","[dbdevicetype][select][byName]")
{
    ...
    //name is unique
    utils::db::DeviceType dbdevicetype =  utils::db::DeviceType(url);
...
TEST_CASE("device type selectId","[dbdevicetype][select][Id]"){
    ...
    int n  =  utils::db::DeviceType::selectId(url,newname);
    REQUIRE( n == 1); //it is an ID
    
    n  =  utils::db::DeviceType::selectId(url,"not found");
    REQUIRE( n == -1); //it is an ID
}
...
TEST_CASE("device type setName","[dbdevicetype][setName]")
{
    ...
    REQUIRE(dbdevicetype.getState() == utils::db::ObjectState::OS_NEW);
    REQUIRE(dbdevicetype.getId() == -1 );
    
    //insert test info
    utils::db::DeviceType newdevicetype = utils::db::DeviceType(url);
    ...
    REQUIRE(dbdevicetype.getState() == utils::db::ObjectState::OS_SELECTED);
    ...
    REQUIRE(dbdevicetype.getState() == utils::db::ObjectState::OS_UPDATED);
    ...
    dbdevicetype.setName(newname);
    REQUIRE(dbdevicetype.getState() == utils::db::ObjectState::OS_DELETED);
...
TEST_CASE("device type insert/delete ","[dbdevicetype][save][insert][delete]"){
    ...
    if ( n == 1 )
    {
        ...
        REQUIRE(utils::db::objectStatetoString(dbdevicetype.getState()) == osselected);
        expected = dbdevicetype.toString();
        n = dbdevicetype.dbsave();
        REQUIRE( n == 0);
        REQUIRE(dbdevicetype.toString() == expected);
        ...
        if ( n == 1 )
        {
            REQUIRE( utils::db::objectStatetoString(dbdevicetype.getState()) == osdeleted);
            REQUIRE( dbdevicetype.getId() == -1 );
...
TEST_CASE("device type update","[dbdevicetype][save][update]"){
    ...
    utils::db::DeviceType dbdevicetype(url,newname);
    int n = dbdevicetype.dbsave();
    REQUIRE( n == 1 );
    ...
    dbdevicetype.setName(newname+newname);
    REQUIRE(utils::db::objectStatetoString(dbdevicetype.getState()) == osupdated);
