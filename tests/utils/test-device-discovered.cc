...
TEST_CASE("Device discovered+getters1","[dbdevice][constructor1][toString][getId][getUrl][getState][getName][getDeviceTypeId][3]")
{
    ...
    REQUIRE( utils::db::objectStatetoString(dbdevice.getState()) == osnew );
...
TEST_CASE("Device discovered+getters2","[dbdevice][constructor2][toString][getId][getUrl][getState][getName][getDeviceTypeId][4]")
{
    ...
    utils::db::DeviceDiscovered dbdevice(url, name);
...
{    
    ...
    std::vector <utils::db::DeviceDiscovered> dbdevices =  
                utils::db::DeviceDiscovered::selectByName(url, newname);
    ...
    utils::db::DeviceDiscovered dbdevice1 = dbdevices.front();
    utils::db::DeviceDiscovered dbdevice2 = dbdevices.back();
    ...
    newname = "not_exist";
    dbdevices =  utils::db::DeviceDiscovered::selectByName(url,newname);
...
TEST_CASE("Device discovered setName/setDeviceType","[dbdevice][setName][setDeviceType][2]")
{
    ...
    REQUIRE(dbdevice.getState() == utils::db::ObjectState::OS_NEW);
    REQUIRE(dbdevice.getId() == -1 );
    
    //insert test info
    utils::db::DeviceDiscovered newdevice = utils::db::DeviceDiscovered(url);
    newdevice.setName(newname);
    newdevice.setDeviceTypeId(1);
    int n = newdevice.dbsave();
    REQUIRE( n == 1 );

    std::vector<utils::db::DeviceDiscovered> dbdevices =  utils::db::DeviceDiscovered::selectByName(url,newname);
    //in DB must be one row for testing
    REQUIRE(dbdevices.size() == 1);
    dbdevice = dbdevices.front();
    REQUIRE(dbdevice.getState() == utils::db::ObjectState::OS_SELECTED);
    ...
    REQUIRE(dbdevice.getState() == utils::db::ObjectState::OS_UPDATED);
    ...
    dbdevice.setName(newname);
    REQUIRE(dbdevice.getState() == utils::db::ObjectState::OS_DELETED);
...
TEST_CASE("Device discovered insert/delete ","[dbdevice][save][insert][delete][3]"){
    ...
    if ( n == 1 )
    {
        ...
        REQUIRE(utils::db::objectStatetoString(dbdevice.getState()) == osselected);
    }
    ...
    if ( n == 1 )
        REQUIRE( utils::db::objectStatetoString(dbdevice.getState()) == osdeleted);
...
TEST_CASE("Device discovered update","[dbdevice][save][update]"){
    ...
    utils::db::DeviceDiscovered dbdevice(url,newname);
    int n = dbdevice.dbsave();
    REQUIRE(n == 0 );
    dbdevice.setDeviceTypeId(1);
    n = dbdevice.dbsave();
    ...
    utils::db::DeviceDiscovered dbdeviceupd(url);
...
TEST_CASE("Device discovered select by Id","[dbdevice][select][byId][1]")
{
    ...
    // select one row
    utils::db::DeviceDiscovered dbdevicesel(url);
    n = dbdevicesel.selectById(dbdevice.getId());
    REQUIRE( n == 1 );
    REQUIRE(utils::db::objectStatetoString(dbdevicesel.getState()) == osselected );
    ...
    //select nothing
    dbdevice = utils::db::DeviceDiscovered(url);
