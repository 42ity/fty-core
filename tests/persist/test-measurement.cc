...
TEST_CASE("measurement INSERT/SELECT/DELETE #1", "[db][CRUD][insert][delete][select][t_bios_measurement][t_bios_measurement_topic]")
{
    ...
    auto ret = select_from_measurement_by_topic(conn, topic1);
    REQUIRE(ret.status == 0);
    REQUIRE(ret.errtype == DB_ERR);
    REQUIRE(ret.errsubtype == DB_ERROR_NOTFOUND);
    REQUIRE(ret.msg);
    ...
    // 3.) SELECT
    ret = select_from_measurement_by_topic(conn, topic1);
    REQUIRE(ret.status == 1);
    REQUIRE(ret.affected_rows == 2);
    ...
    // 4.) SELECT all
    ret = select_from_measurement_by_topic(conn, "topic");
    REQUIRE(ret.status == 1);
    REQUIRE(ret.affected_rows == 3);
