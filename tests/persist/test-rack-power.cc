...
TEST_CASE("Rack power #1","[db][power][rack][calc][rack_power.sql]")
{
    ...
    int rv = 0; int rv1 = 0; int rv2 = 0;
    REQUIRE_NOTHROW( rv = compute_result_value_get(results, &value) );
    REQUIRE_NOTHROW( rv1 = compute_result_scale_get(results, &scale) );
    REQUIRE_NOTHROW( rv2 = compute_result_num_missed_get(results, &num_missed) );
...
//    REQUIRE_NOTHROW ( yy = convert_asset_to_monitor(url.c_str(), 8114));
 ...
 //   ids.insert(yy);
    ...
    int rv = 0; int rv1 = 0; int rv2 = 0;
    REQUIRE_NOTHROW( rv = compute_result_value_get(results, &value) );
    REQUIRE_NOTHROW( rv = compute_result_scale_get(results, &scale) );
    REQUIRE_NOTHROW( rv = compute_result_num_missed_get(results, &num_missed) );
...
TEST_CASE("Rack power #10","[db][power][rack][calc][rack_power.sql]")
{
    ...
    // expected devices to summ up
    std::set <m_dvc_id_t > expected_ids;
    expected_ids.insert(convert_asset_to_monitor(url.c_str(), 8144));
...
TEST_CASE("Rack power #1 v2","[db][power][rack][calc][v2][rack_power.sql]")
{
    ...
    std::map<std::string, std::set<std::string>> const EXP {
        ...
        { "RACK3",   { "epdu3_1", "ups3_1", "ups3_2", "ups3_3" } },
