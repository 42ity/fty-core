...
TEST_CASE("f:select_alertDates_byRuleName_byInterval_byDcId #1","[db][outage][select][test_outage.sql][1]")
{
    log_open ();
    log_info ("=============== OUTAGE #1 ==================");
...
/*
TEST_CASE("f:calculate_outage_byInerval_byDcId #2","[db][calculate][select][test_outage.sql][2]")
{
    log_open ();
    log_info ("=============== OUTAGE #2 ==================");
    ...
    int64_t outage = 0;
    int rv = calculate_outage_byInerval_byDcId (conn, start_date, end_date, dc_id, outage);
