...
trap '[ -n "$WEBTESTPID" -a -d "/proc/$WEBTESTPID" ] && echo "INFO: Killing make web-test PID $WEBTESTPID to exit" && kill "$WEBTESTPID" killall tntnet 2>/dev/null || true; sleep 1; ps -ef | grep -v grep | grep tntnet && ps -ef | grep -v grep | egrep "$$|make" && echo "CI-ERROR: tntnet still alive">&2 && killall -9 tntnet && exit 1' 0 1 2 3 15
