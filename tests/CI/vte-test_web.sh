...
#  \brief  This script automates remote tests of REST API for the $BIOS project
...
# server and is started on another server - MS (test Management Station) from
# a checked-out copy of the bios-core development workspace
#
# ***** ABBREVIATIONS *****
...
if [ "$SKIP_SANITY" = yes ]; then
    ...
    if ! sut_run "pidof malamute > /dev/null"; then
        logmsg_error "malamute is not running (on remote SUT), you may need to start it first!"
    fi

    if ! sut_run "pidof mysqld > /dev/null" ; then
        logmsg_error "mysqld is not running (on remote SUT), you may need to start it first!"
    fi
    ...
    logmsg_info "The remote SUT webserver seems basically able to serve the REST API"
fi
