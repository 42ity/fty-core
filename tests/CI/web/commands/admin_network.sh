...
do_test "admin_network_bad_method" api_get /admin/network \
    'HTTP/1.1 404 Not Found'
print_result $?

do_test "admin_network_bad_request" api_auth_post /admin/network \
    '{"net" : "123"}' \
...
do_test "admin_network_post_excl1" api_auth_post /admin/network \
    '{"net" : "10.20.30.40/24", "status" : "E"}' \
    '{ "id" : "5" }'
print_result $?

# check the double POST just returns the same ID
do_test "admin_network_post_excl2" api_auth_post /admin/network \
    '{"net" : "10.20.30.40/24", "status" : "E"}' \
    '{ "id" : "5" }'
print_result $?

# ... but manual status is different
do_test "admin_network_post_man1" api_auth_post /admin/network \
    '{"net" : "10.20.30.40/24", "status" : "M"}' \
    '{ "id" : "6" }'
print_result $?

do_test "admin_network_post_man2" api_auth_post /admin/network \
    '{"net" : "10.20.30.40/24", "status" : "M"}' \
    '{ "id" : "6" }'
print_result $?
