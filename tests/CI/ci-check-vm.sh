...
remote_make() {
    ...
    if ssh root@$VM -p $PORT "cd $BCHECKOUTDIR && [ -s ${MAKELOG} ]" ; then
        ...
        ssh root@$VM -t -p $PORT "/bin/bash --login -x -c 'set -o pipefail ; cd $BCHECKOUTDIR && { ./autogen.sh --nodistclean ${AUTOGEN_ACTION_MAKE} install 2>&1 | tee -a ${MAKELOG}; } && echo === remote build finished ok'"
        return $?
    else
        # Newly fetched branch - clean up, configure and make it fully
        echo "-- compiling to rebuild"
        ssh root@$VM -t -p $PORT "/bin/bash --login -x -c 'set -o pipefail ; cd $BCHECKOUTDIR && { eval ./autogen.sh --configure-flags \"--prefix=\$HOME\ --with-saslauthd-mux=/var/run/saslauthd/mux\" ${AUTOGEN_ACTION_BUILD} install 2>&1 | tee ${MAKELOG}; } && echo === remote build finished ok'"
        return $?
    fi
}
