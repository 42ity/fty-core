...
if isRemoteSUT ; then
    ...
    SERVICES="$(sut_run 'ls -1 /etc/systemd/system/bios.target.wants/*.service | egrep -v "malamute|db-init|bios-networking" | while read F ; do echo "`basename "$F"`"; done' )"
...
SERVICES="$(ls -1 /etc/systemd/system/bios.target.wants/*.service | egrep -v 'malamute|db-init|bios-networking' | while read F ; do BF="`basename "$F"`"; [ -s "$CHECKOUTDIR/systemd/$BF.in" ] || echo "$BF"; done)"
