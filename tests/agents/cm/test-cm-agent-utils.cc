...
TEST_CASE ("process_db_measurement_json_to_map","[agent-cm][computation][average][db]")
{
    ...
    samples.emplace (std::make_pair (12, 0.01423));

    SECTION ("bad args 1 - garbage") {
        CHECK ( process_db_measurement_json_to_map (NULL, samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        CHECK ( process_db_measurement_json_to_map ("", samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        CHECK ( process_db_measurement_json_to_map ("{}", samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        CHECK ( process_db_measurement_json_to_map ("x", samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
    }

    SECTION ("bad args 2 - various syntactically correct but bad jsons") {
        // Empty data member
        std::string json =
        "{"
        "   \"unit\" : \"C\","
        ...
        "]"
        "}";
        CHECK ( process_db_measurement_json_to_map (json.c_str (), samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        // Bad data member type
        json =
        "{"
        "   \"unit\" : \"C\","
        "   \"source\" : \"temperature.thermal_zone0\","
        ...
        "   \"end_ts\" : 1426208405,"
        "   \"data\" : ["
        "{ \"value\": 510, \"scale\": -1, \"timestamp\": 1426205760 },"
        "{ \"value\": 100, \"scale\": -1, \"timestamp\": 1426208100 }"
        "]"
        "}";
        CHECK ( process_db_measurement_json_to_map (json.c_str (), samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        ...
        json =
        "{"
        "   \"sources\" : \"temperature.thermal_zone0\","
        "   \"unit\" : \"C\","
        "   \"element_id\" : 26,"
        "   \"start_ts\" : 1426201800,"
        "   \"end_ts\" : 1426208405,"
        ...
        CHECK ( process_db_measurement_json_to_map (json.c_str (), samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        // start_ts -  array instead of number
        json =
        "{"
        "   \"sources\" : \"temperature.thermal_zone0\","
        "   \"unit\" : \"C\","
        ...
        CHECK ( process_db_measurement_json_to_map (json.c_str (), samples, st, end, eid, source, unit) == -1 );
        CHECK ( samples.size () == 0 );
        // element_id - non-convertible string 
        json =
        "{"
        "   \"sources\" : \"temperature.thermal_zone0\","
        "   \"unit\" : \"C\","
        ...
        "   \"end_ts\" : 1426208405,"
        "   \"data\" : ["
        "{ \"value\": 510, \"timestamp\": 1426205760 },"
        "{ \"value\": 100, \"scale\": -1, \"timestamp\": 1426208100 }"
        "]"
        "}";
        CHECK ( process_db_measurement_json_to_map (json.c_str (), samples, st, end, eid, source, unit) == -1 );
    ...
    SECTION ("correct invocation") {
        ...
        "{"
        "   \"unit\" : \"Zz\","
        "   \"source\" : \"temperatura.termalny_pramen.piestany\","
        "   \"element_id\" : 1335,"
        "   \"start_ts\" : 14,"
        "   \"end_ts\" : 1426208407,"
        "   \"data\" : ["
        "       { \"value\": 420, \"scale\" : -1, \"timestamp\" : 1426204882 },"
        "       { \"value\": 11080, \"scale\" : -2, \"timestamp\" : 1426207962 },"
        "       { \"value\": 206489, \"scale\" : -3, \"timestamp\" : 1427254783 }"
        "]"
        "}";
        CHECK ( process_db_measurement_json_to_map (json.c_str (), samples, st, end, eid, source, unit) == 0 );
        CHECK ( st == 14 );
        CHECK ( end == 1426208407 );
        CHECK ( eid == 1335 );
        CHECK ( unit  == "Zz" );
        CHECK ( source == "temperatura.termalny_pramen.piestany" );
        CHECK ( samples.size () == 3 );
        CHECK ( samples.at (1426204882) == 42.0 );
        CHECK ( samples.at (1426207962) == 110.8 );
        CHECK ( samples.at (1427254783) == 206.489 );
    }
}
