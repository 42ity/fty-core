...
/* Update existing 'asset_order' Ext attribute :
   Fix rack asset order format (expect 8 numbers before the dot (000XXXXX.XXXXX) */
UPDATE t_bios_asset_ext_attributes SET value =
CASE WHEN INSTR(value, '.') != 0 THEN CONCAT(LPAD(SUBSTRING_INDEX(value, '.', 1), 8, '0'), '.', SUBSTRING_INDEX(value, '.', -1))
ELSE LPAD(value, 8, '0') END
WHERE keytag = "asset_order";
