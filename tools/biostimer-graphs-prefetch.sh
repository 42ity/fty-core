...
# Now this is done by lots of (throttled) calls to a binary that manipulates
# data in the database, one call for each pre-calculation pattern (data-type,
# time-step, function).
#
# PURPOSE:
# Have a cron job or a systemd timer unit periodically call this script
...
# Beside HTTP support, SUT_HOST is also used in sut_run() and so in do_select()
[ -z "${SUT_HOST-}" ] || \
        echo "WARNING: $0: SUT_HOST was set to '$SUT_HOST', however we only support 'localhost'!" >&2
SUT_HOST="127.0.0.1"
SUT_IS_REMOTE="no"
export SUT_HOST SUT_IS_REMOTE
...
{ [ -s "/usr/share/bios/scripts/scriptlib.sh" ] &&
        . "/usr/share/bios/scripts/scriptlib.sh" ; } || \
...
# Legacy naming from the past of HTTP-based implementation
[ -z "${FETCHER-}" ] && FETCHER=""
[ -z "${FETCHER-}" ] && \
for F in \
        "$SCRIPTDIR/../average" \
        "$SCRIPTDIR/average" \
        "/usr/libexec/bios/average" \
; do
        [ -s "$F" ] && [ -x "$F" ] && FETCHER="$F" && break
done
[ -n "${FETCHER-}" ] && [ -s "$FETCHER" ] && [ -x "$FETCHER" ] || \
        logmsg_warn "Can not find 'average' pre-calculator program, wet-run mode would fail!"
...
start_lock() {
    ...
    [ -f "$LOCKFILE" ] && \
        die 0 "A copy of the script seems already running ('$LOCKFILE' exists):" "`cat "$LOCKFILE"`"
...
generate_prefetchcmd_strings() {
    generate_prefetchcmd_strings_sources
}
...
generate_prefetchcmd_strings_sources() {
    ...
    for i in $element_ids; do
        ...
        # 5. Generate prefetch request for each combination of source, step, type for the given element id
        for s in $SOURCES; do
            ...
            for stype in $TYPES; do
                for sstep in $STEPS; do
                    echo "${FETCHER} 'start_ts=${START_TIMESTAMP}&end_ts=${END_TIMESTAMP}&type=${stype}&step=${sstep}&element_id=${i}&source=$s'"
    ...
    logmsg_info "Successfully generated $NUM_STRINGS request command-lines for power sources" >&2
    return 0
}
...
FIRED_BATCH=""
run_prefetchcmd_strings() {
    # This is a write-possible operation that updates timestamp files.
    # Executes a series of fetch-lines from generate_prefetchcmd_strings()
    ...
    if [ "$MAX_CHILDREN" -gt 1 ] 2>/dev/null; then
        run_prefetchcmd_strings_parallel "$@"
    else
        run_prefetchcmd_strings_sequential "$@"
    fi < <(generate_prefetchcmd_strings)
...
run_prefetchcmd_strings_sequential() {
    # Internal detail for run_prefetchcmd_strings()
    logmsg_info 0 "`date`: Using sequential foreground prefetcher requests" >&2
...
run_prefetchcmd_strings_parallel() {
    # Internal detail for run_prefetchcmd_strings()
    logmsg_info 0 "`date`: Using parallel background prefetcher requests (at most $MAX_CHILDREN at once)" >&2
    while IFS="" read CMDLINE; do
        [ "$RES" -lt 0 ] && RES=0
        COUNT_TOTAL=$(($COUNT_TOTAL+1))
...
case "$ACTION" in
    ...
    request-quiet) # default / quiet mode for timed runs
        ...
        [ -z "$FETCHER" ] && \
                die "No usable FETCHER was detected on this system"
        run_prefetchcmd_strings "$@" >/dev/null
        exit $?
        ;;
esac
