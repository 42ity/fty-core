...
AUGOUT="`(echo 'match /files/etc/network/interfaces/iface[*]'; echo 'match /files/etc/network/interfaces/iface[*]/method' ; echo 'match /files/etc/default/ifplugd/INTERFACES' ) | augtool -S -I/usr/share/fty/lenses`" || die $?
...
if [ "$OLDINTLIST" = "$INTLIST" ]; then
    ...
    if ! /bin/echo -E set /files/etc/default/ifplugd/INTERFACES "\"\\\"$INTLIST\\\"\"" | augtool -S -I/usr/share/fty/lenses -e -s ; then
        [ -n "$OLDINTLIST" ] && ifplugd_on
        die "Could not apply changes to ifplugd config"
    fi
    if ! /bin/echo -E set /files/etc/default/networking/EXCLUDE_INTERFACES "\"\\\"$INTLIST\\\"\"" | augtool -S -I/usr/share/fty/lenses -e -s ; then
